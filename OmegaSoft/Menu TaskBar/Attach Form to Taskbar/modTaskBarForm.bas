...
'  2.  Call the 'AttachForm' function to set the parent window of your form to be that Shell_TrayWnd
'       ---When the taskbar is horizontal
'           -the intForceWidth parameter specifies the maximum width of the form
'           -the intForceHeight parameter spcifies the height (unless the taskbar is smaller)
'           -the blnGrowWithTray allows the height to grow but maintains the width
'       ---When the taskbar is vertical
'           -the intForceWidth parameter specifies the width of the form (unless the taskbar is smaller)
'           -the intForceHeight parameter spcifies the maximum height of the form
'           -the blnGrowWithTray allows the width to grow but maintains the height
'  3. Don't forget to use the Form_Resize and Form_Paint functions to size the form controls

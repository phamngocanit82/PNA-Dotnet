...
<ToolboxBitmap(GetType(ButtonStyle4PNA), "ButtonStyle4PNA.bmp")> _
Public Class ButtonStyle4PNA
#Region "ClassButtonStyle4PNA"
#Region "Enum/Struct"
        ...
        Try
            ...
            Select Case _state
                Case StateButton.Normal, StateButton.MouseMove
                    If Enabled Then
                        If Not Image Is Nothing Then g.DrawImage(Image, Width - Image.Width - 6, CInt(Height / 2) - CInt(Image.Height / 2) - 1)
                        g.DrawString(Text, Font, New SolidBrush(ForeColor), rc, sf)
                    Else
                        If Not Image Is Nothing Then ControlPaint.DrawImageDisabled(g, Image, Width - Image.Width - 6, CInt(Height / 2) - CInt(Image.Height / 2) - 1, BackColor)
                        g.DrawString(Text, Font, New SolidBrush(Color.FromArgb(161, 161, 146)), rc, sf)
                    End If
                Case StateButton.Press
                    If Not Image Is Nothing Then g.DrawImage(Image, Width - Image.Width - 5, CInt(Height / 2) - CInt(Image.Height / 2))
                    rc.Width = rc.Width + 1
                    g.DrawString(Text, Font, New SolidBrush(ForeColor), rc, sf)
            End Select
        Catch ex As Exception
...
#Region "Style1"
    Private Sub _style1Normal(ByVal g As Graphics)
        Try
            ...
            g.DrawLine(New Pen(Color.White), Width - 2, 1, Width - 2, Height - 2)
            g.DrawLine(New Pen(Color.White), Width - 2, Height - 2, 3, Height - 2)
            ...
            g.DrawLine(New Pen(Color.FromArgb(121, 157, 182)), Width - 1, 0, Width - 1, Height - 1)
            g.DrawLine(New Pen(Color.FromArgb(121, 157, 182)), Width - 1, Height - 1, 1, Height - 1)
    ...
    Private Sub _style1Press(ByVal g As Graphics)
        Try
            ...
            g.DrawLine(New Pen(Color.FromArgb(147, 125, 90)), Width - 1, 0, Width - 1, Height - 1)
            g.DrawLine(New Pen(Color.FromArgb(147, 125, 90)), Width - 1, Height - 1, 1, Height - 1)
...
#End Region
#Region "Events"
    Private Sub ButtonStyle4PNA_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.GotFocus
        _bolFocus = True
        Invalidate()
    End Sub
    Private Sub ButtonStyle4PNA_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.LostFocus
        _bolFocus = False
        Invalidate()
    End Sub
    ...
    Protected Overrides Sub OnResize(ByVal e As System.EventArgs)
        Try
            ...
            gp.AddLine(CInt(Width / 4), 0, Width - 1, 0)
            gp.AddLine(Width - 1, 0, Width, 1)
            gp.AddLine(Width, 1, Width, Height - 2)
            gp.AddLine(Width, Height - 2, Width - 1, Height - 1)
            gp.AddLine(Width - 1, Height - 1, 0, Height)

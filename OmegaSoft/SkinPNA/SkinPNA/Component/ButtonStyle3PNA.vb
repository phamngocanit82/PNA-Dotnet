...
<ToolboxBitmap(GetType(ButtonStyle3PNA), "ButtonStyle3PNA.bmp")> _
Public Class ButtonStyle3PNA
#Region "ClassButtonStyle3PNA"
#Region "Enum/Struct"
        ...
        Try
            ...
            Select Case _state
                Case StateButton.Normal, StateButton.MouseMove
                    If Enabled Then
                        If Not Image Is Nothing Then g.DrawImage(Image, 5, CInt(Height / 2) - CInt(Image.Height / 2) - 1)
                        g.DrawString(Text, Font, New SolidBrush(ForeColor), rc, sf)
                    Else
                        If Not Image Is Nothing Then ControlPaint.DrawImageDisabled(g, Image, 5, CInt(Height / 2) - CInt(Image.Height / 2) - 1, BackColor)
                        g.DrawString(Text, Font, New SolidBrush(Color.FromArgb(161, 161, 146)), rc, sf)
                    End If
                Case StateButton.Press
                    If Not Image Is Nothing Then g.DrawImage(Image, 6, CInt(Height / 2) - CInt(Image.Height / 2))
                    rc.Width = rc.Width + 1
                    g.DrawString(Text, Font, New SolidBrush(ForeColor), rc, sf)
            End Select
        Catch ex As Exception
...
#Region "Style1"
    Private Sub _style1Normal(ByVal g As Graphics)
        Try
            ...
            _drawTextAndImage(g)
            'Border
            g.DrawLine(New Pen(Color.White), 1, 1, Width - CInt(Width / 4) - 1, 1)
            g.DrawLine(New Pen(Color.White), Width - CInt(Width / 4) - 1, 1, Width - 4, Height - 2)
            g.DrawLine(New Pen(Color.White), Width - 4, Height - 2, 2, Height - 2)
            g.DrawLine(New Pen(Color.White), 1, Height - 2, 1, 1)
            ...
            g.DrawLine(New Pen(Color.FromArgb(121, 157, 182)), Width - 2, Height - 1, 1, Height - 1)
            g.DrawLine(New Pen(Color.FromArgb(121, 157, 182)), 0, Height - 2, 0, 1)
        Catch ex As Exception
        End Try
    End Sub
    ...
    Private Sub _style1Press(ByVal g As Graphics)
        Try
            ...
            g.DrawLine(New Pen(Color.FromArgb(147, 125, 90)), Width - 2, Height - 1, 1, Height - 1)
            g.DrawLine(New Pen(Color.FromArgb(147, 125, 90)), 0, Height - 2, 0, 1)
        Catch ex As Exception
        End Try
    End Sub
...
#End Region
#Region "Events"
    Private Sub ButtonStyle3PNA_GotFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.GotFocus
        _bolFocus = True
        Invalidate()
    End Sub
    Private Sub ButtonStyle3PNA_LostFocus(ByVal sender As Object, ByVal e As System.EventArgs) Handles MyBase.LostFocus
        _bolFocus = False
        Invalidate()
    End Sub
    ...
    Protected Overrides Sub OnResize(ByVal e As System.EventArgs)
        Try
            ...
            gp.AddLine(0, 1, 1, 0)
            gp.AddLine(1, 0, Width - CInt(Width / 4), 0)
            gp.AddLine(Width - CInt(Width / 4), 0, Width, Height)
            gp.AddLine(Width, Height, 1, Height)

...
<ToolboxBitmap(GetType(ButtonNoSound), "NoSound0.png")> _
Public Class ButtonNoSound
#Region "ClassButtonNoSound"
#Region "Enum/Struct"
        ...
        Try
            ...
            Select Case _style
                Case StyleButton.Style1
                    _img(0) = New Bitmap(AppAssembly.GetManifestResourceStream("OmegaMusic.NoSound0.png"))
                    _img(1) = New Bitmap(AppAssembly.GetManifestResourceStream("OmegaMusic.NoSound1.png"))
                    _img(2) = New Bitmap(AppAssembly.GetManifestResourceStream("OmegaMusic.NoSound2.png"))
...
#End Region
#Region "Events"
    Private Sub ButtonNoSound_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Width = 21 : Height = 21
        _getResBitmap()
        Refresh()
    End Sub
    ...
    Protected Overrides Sub OnResize(ByVal e As System.EventArgs)
        Try
            ...
            gp.AddLine(0, 1, 2, 0)
            gp.AddLine(2, 0, Width - 2, 0)
            gp.AddLine(Width - 2, 0, Width - 1, 1)
            gp.AddLine(Width - 1, 1, Width, 2)
            gp.AddLine(Width, 2, Width, Height - 2)
            gp.AddLine(Width, Height - 2, Width - 1, Height - 2)
            gp.AddLine(Width - 1, Height - 2, Width - 2, Height - 1)
            gp.AddLine(Width - 2, Height - 1, Width - 2, Height)
            gp.AddLine(Width - 2, Height, 2, Height)
            gp.AddLine(2, Height, 2, Height - 1)
            gp.AddLine(2, Height - 1, 1, Height - 2)
            gp.AddLine(1, Height - 2, 0, Height - 2)
            gp.AddLine(0, Height - 2, 0, 2)
            gp.AddLine(0, 2, 1, 1)
            gp.AddLine(1, 1, 2, 0)
